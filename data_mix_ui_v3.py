...
def export_shards(df, output_path, shard_size_gb=1):
    ...
    for idx, row in df.iterrows():
        ...
        written_bytes += sample_bytes
        
        # 更新进度
        if idx % 1000 == 0:
            progress = min(written_bytes / total_bytes, 1.0)
            progress_bar.progress(progress)
            status_text.text(f"写入样本 {idx+1}/{len(df)} | 已写入: {written_bytes/GB:.2f}GB | 当前分片: {shard_idx}")
        ...
        with st.spinner("🔍 正在扫描数据集文件..."):
            try:
                ...
                with ThreadPoolExecutor(max_workers=8) as executor:
                    future_to_file = {executor.submit(parse_jsonl_file, file): file for file in jsonl_files}

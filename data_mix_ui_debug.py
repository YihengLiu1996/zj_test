...
# --- 修复核心问题：统一处理模式状态管理 ---
# 1. 定义一致的模式标识符
MODE_MEMORY = "内存模式（小数据）"
MODE_STREAMING = "流式模式（大数据）"

# 2. 初始化处理模式状态 (使用统一的标识符)
if 'processing_mode' not in st.session_state:
    st.session_state.processing_mode = MODE_MEMORY # 默认值

# 3. 数据处理模式选择 (与 session_state 同步)
#    使用 st.session_state.processing_mode 的值来确定 radio 的默认选项
selected_mode = st.sidebar.radio(
    "处理模式",
    [MODE_MEMORY, MODE_STREAMING], # 使用统一的标识符
    index=0 if st.session_state.processing_mode == MODE_MEMORY else 1,
    help="内存模式适用于<100GB数据，流式模式适用于>100GB数据"
)
# 4. 更新 session_state (如果用户改变了选择)
...
if st.sidebar.button("📁 加载数据集", type="primary"):
    if not data_path:
        ...
        with st.spinner("🔍 正在扫描数据集文件..."):
            try:
                # --- 修复：使用统一的模式标识符进行判断 ---
                if st.session_state.processing_mode == MODE_MEMORY:
                    # 原有的内存模式处理
                    jsonl_files = []
                    ...
                    if all_data:
                        ...
                        # --- 修复：使用统一的模式标识符存储状态 ---
                        st.session_state.processing_mode = MODE_MEMORY # 确保一致
                        # ----------------------------------------
                        ...
                        st.stop()
                else:  # 流式模式 (st.session_state.processing_mode == MODE_STREAMING)
                    # 大数据流式处理
                    ...
                    # --- 修复：使用统一的模式标识符存储状态 ---
                    st.session_state.processing_mode = MODE_STREAMING # 确保一致
                    # -----------------------------------------
...
# 检查数据是否已加载
# --- 修复：使用更明确的条件检查 ---
#     检查 session_state 中是否存在对应模式所需的数据
if (st.session_state.processing_mode == MODE_MEMORY and 'df' in st.session_state) or \
   (st.session_state.processing_mode == MODE_STREAMING and 'sampler' in st.session_state):
    # ========== 配比调整配置 ==========
    ...
    else:  # streaming mode (st.session_state.processing_mode == MODE_STREAMING)
                ...
                for i_offset, val in enumerate(values[i_start:i_start + items_per_row]):
                    ...
                    with cols[i_offset]:
                        ratio = st.number_input(
                            ...
                            step=0.01,
                            format="%.3f",
                            key=f"{dim}_{val}_stream" # 添加后缀避免key冲突
                        )
                        st.session_state.target_ratios[dim][val] = ratio
                        target_ratios[dim][val] = ratio
                        total_ratio += ratio
    ...
    if st.session_state.processing_mode == MODE_MEMORY:
        ...
        with col1:
            ...
            ax.axis('equal')
            st.pyplot(fig)
            plt.close(fig) # Close figure to prevent resource leaks
        # 2. Category 配比图
        with col2:
